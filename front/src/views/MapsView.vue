...
const maps: Ref<Map[], Map[]> = ref([])

onMounted(async () => {
  loading.value = true
  try {
    const response = await fetch("http://localhost:4000/maps")
    if (response.ok) {
...
<template>
  <main class="bg-gradient-to-b from-gray-800 to-gray-900 text-white p-6">
    <div class="flex flex-col items-center justify-start min-h-screen gap-6">
      <h1 class="text-3xl font-title font-semibold text-center">Liste des maps</h1>
